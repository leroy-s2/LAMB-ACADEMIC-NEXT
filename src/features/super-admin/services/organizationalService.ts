...
/**
 * Obtiene unidades organizativas por tipo
 * GET /unidades-organizativas/tipo-unidad/{tipoUnidadId}
 */
export async function getUnidadesByTipo(tipoUnidadId: number): Promise<UnidadOrganizativaResponse[]> {
  try {
    const response = await api.get<BackendResponse<UnidadOrganizativaResponse[]>>(
      `/unidades-organizativas/tipo-unidad/${tipoUnidadId}`
    );
    ...
    throw new Error(response.data.message || 'Error al obtener unidades por tipo');
  } catch (error: any) {
    console.error('❌ Error getUnidadesByTipo:', error);
    throw error;
  }
}
 ...
 * GET /unidades-organizativas/hijas/{unidadPadreId}
 */
export async function getUnidadesHijas(unidadPadreId: number): Promise<UnidadOrganizativaResponse[]> {
  try {
    const response = await api.get<BackendResponse<UnidadOrganizativaResponse[]>>(
      `/unidades-organizativas/hijas/${unidadPadreId}`
    );
    ...
    throw new Error(response.data.message || 'Error al obtener unidades hijas');
  } catch (error: any) {
    console.error('❌ Error getUnidadesHijas:', error);
    throw error;
  }
}
